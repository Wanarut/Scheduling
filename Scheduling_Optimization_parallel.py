...
def main():
    ...
    solution_0 = np.zeros((chromosome_length, 2), int)
    tasks_0 = tasks.copy()
    tasks_0 = PDM_calculation(tasks_0, solution_0)
    tasks_0 = pd.DataFrame(tasks_0,columns=tasks.columns.values.tolist())
    ...
    print('Mx', mx_0, 'man^2')
    print('> use' , tpp, 'sec/sol -> estimated time left', pd.to_timedelta(maximum_population_size*maximum_generation*tpp*9/cpu_core, unit='s'))
    # return 0
...
def create_population(solutions_size, chromosome_length, constraints):
    """
    Create random population with given number of solutions and chroosome length.
    """
    # Set up an initial array of all zeros
    population = np.zeros((solutions_size, chromosome_length, 2), int)

    # Loop through each row (solution)
...
# Fitness score
def score_population(display, tasks, costs, population, index, return_dict):
    ...
    for i in range(population_size):
        ...
        shift_tasks = PDM_calculation(shift_tasks, population[i])
        shift_tasks = pd.DataFrame(shift_tasks,columns=tasks.columns.values.tolist())
...
def mx_per_day(tasks, i):
    ...
    cur_job = labour_resource[(cur_day >= Early_Start) & (cur_day <= Early_Finish)]
    cur_job = cur_job[pd.notnull(cur_job)]
    # return np.sum(cur_job)
    result = math.pow(np.sum(cur_job), 2)
    # print(i, timer()-start)
    return result
...
# PDM network
def PDM_Forward(i, tasks, solution):
    # Calculated
    if pd.notnull(tasks.at[i, 'Early_Start']):
        return
    
    shiftday = solution[i, 0]
    option = solution[i, 1]
...
def PDM_Backward(i, tasks, solution):
...
def PDM_calculation(tasks, solution):
    tasks_length = tasks.shape[0]
    for i in range(tasks_length):
        PDM_Forward(i, tasks, solution)
        PDM_Backward(tasks_length - i - 1, tasks, solution)
